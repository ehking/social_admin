...
@pytest.fixture
def legacy_engine_without_media_url(tmp_path, monkeypatch):
    db_path = tmp_path / "legacy-no-media-url.db"
    engine = create_engine(
        f"sqlite:///{db_path}", connect_args={"check_same_thread": False}
    )
...
def test_run_startup_migrations_adds_media_url_column(
    legacy_engine_without_media_url,
):
    database.run_startup_migrations()

    with legacy_engine_without_media_url.begin() as connection:
        columns = connection.execute(text("PRAGMA table_info(job_media)")).fetchall()
        column_names = {column[1] for column in columns}
